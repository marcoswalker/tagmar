export default class tagmarActorSheet extends ActorSheet {
    ...
    get template() {
        ...
        if (this.document.type == "Personagem" && layout != "base") {
            ...
            return 'systems/tagmar/templates/sheets/'+ this.document.type.toLowerCase() +'-sheet.hbs';
        }
    }
    async getData(options) {
        ...
        if (data.document.type == "Inventario") {
            ...
            if (data.document.items.filter(item => item.type == "Raca")[0]) {
                ...
                if (!game.settings.get('tagmar', 'ajusteManual')) actorUtils._calculaAjuste(data, updatePers);
